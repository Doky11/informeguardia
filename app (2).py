...
for categoria in categorias:
    ...
    observacion_categoria = st.text_area(f"Observaciones para {categoria}", value=str(nota))
    st.write(f"Nota numérica: **{nota}**, Nota en letra: **{letra}**")
    resultados[categoria] = (nota, letra, observacion_categoria)
    total_nota += nota
        ...
        for fila in tabla.rows:
            for i, celda in enumerate(fila.cells):
                ...
                if "INFORMANTE" in texto and i < len(fila.cells) - 1:
                    ...
                    fila.cells[i].text = f"OBSERVACIONES GENERAL / JUSTIFICACIÓN
